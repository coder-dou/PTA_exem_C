...
其中准考证号由16位数字组成，座位从 1 到 N 编号。输入保证每个人的准考证号都不同，并且任何时候都不会把两个人分配到同一个座位上。
考生信息之后，给出一个正整数 M（≤N），随后一行中给出 M 个待查询的  试机座位  号码，以空格分隔。
输出格式：
...
int main(){
	...
	scanf("%d",&N);
	int a,b;//a 试机座位, b 考试座位
	for(i=0;i<N;i++){
		scanf("%s %d %d",arrStr[i],&a,&b);
		arrStr[i][16]='\0';
		arr[a][0]=i;
		arr[a][1]=b;
	}
